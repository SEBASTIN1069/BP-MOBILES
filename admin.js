...
function loadProductsTable() {
  ...
  _unsubProducts = db.collection("products").orderBy("createdAt", "desc")
    .onSnapshot((snap) => {
      ...
      const totalPages = Math.max(1, Math.ceil(allProductsData.length / ADMIN_PAGE_SIZE));
...
function renderProductsPage() {
  ...
  const pageItems = allProductsData.slice(start, start + ADMIN_PAGE_SIZE);
  ...
  renderPaginationControls(productPagination, allProductsData.length, productPage, (page) => {
    productPage = page;
    renderProductsPage();
  });
}
...
function loadAccessoriesTable() {
  ...
  _unsubAccessories = db.collection("accessories").orderBy("createdAt", "desc")
    .onSnapshot((snap) => {
      ...
      const totalPages = Math.max(1, Math.ceil(allAccessoriesData.length / ADMIN_PAGE_SIZE));
...
function renderAccessoriesPage() {
  ...
  const pageItems = allAccessoriesData.slice(start, start + ADMIN_PAGE_SIZE);
  ...
  renderPaginationControls(accessoryPagination, allAccessoriesData.length, accessoryPage, (page) => {
    accessoryPage = page;
    renderAccessoriesPage();
  });
}
...
function loadLaptopsTable() {
  ...
  _unsubLaptops = db.collection("laptops").orderBy("createdAt", "desc")
    .onSnapshot((snap) => {
      ...
      const totalPages = Math.max(1, Math.ceil(allLaptopsData.length / ADMIN_PAGE_SIZE));
...
function renderLaptopsPage() {
  if (allLaptopsData.length === 0) {
    laptopTableBody.innerHTML = `<tr><td colspan="6">No laptops added yet.</td></tr>`;
    laptopPagination.innerHTML = "";
    return;
  }

  const start = (laptopPage - 1) * ADMIN_PAGE_SIZE;
  const pageItems = allLaptopsData.slice(start, start + ADMIN_PAGE_SIZE);
  ...
  renderPaginationControls(laptopPagination, allLaptopsData.length, laptopPage, (page) => {
    laptopPage = page;
    renderLaptopsPage();
  });
}
