...
// ---- Accounts widget: shows total signups + searchable mobile number list ----
// Reads the "customers" collection (created by the public site's signup
// form — see script.js). Read-only here: this widget never writes,
// updates, or deletes any customer document. Data loads only when the
// admin actually clicks "Total Accounts" — not automatically on page load.
...
let accountsListenerStarted = false;

function startAccountsListener() {
  if (accountsListenerStarted) return;
  accountsListenerStarted = true;
  accountsCount.textContent = "…";
  db.collection("customers").onSnapshot((snap) => {
    allCustomersData = snap.docs.map((doc) => ({ id: doc.id, ...doc.data() }));
    accountsCount.textContent = allCustomersData.length;
    renderAccountsList();
  }, () => {
    accountsCount.textContent = "!";
  });
...
function renderAccountsList() {
  const filtered = customersSearchQuery
    ? allCustomersData.filter((c) =>
        (c.name || "").toLowerCase().includes(customersSearchQuery) ||
        (c.mobile || c.id).includes(customersSearchQuery))
    : allCustomersData;

  if (filtered.length === 0) {
    accountsList.innerHTML = `<p class="admin-accounts-empty">${customersSearchQuery ? "No matching accounts." : "No accounts created yet."}</p>`;
    return;
  }

  accountsList.innerHTML = filtered.map((c) => `
    <div class="admin-accounts-row">
      <span class="admin-accounts-name">${c.name || "—"}</span>
      <span class="admin-accounts-mobile">${c.mobile || c.id}</span>
    </div>
  `).join("");
}

accountsSearchInput.addEventListener("input", () => {
  customersSearchQuery = accountsSearchInput.value.trim().toLowerCase();
  renderAccountsList();
});

accountsToggle.addEventListener("click", () => {
  accountsPanel.hidden = !accountsPanel.hidden;
  if (!accountsPanel.hidden) startAccountsListener();
});

// Close the accounts panel when clicking anywhere outside it
document.addEventListener("click", (e) => {
  if (!accountsPanel.hidden && !e.target.closest(".admin-accounts-widget")) {
