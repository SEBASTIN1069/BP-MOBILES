...
// ===== Render: accessories on products page (full catalogue) =====
const accessoryGridFull = document.getElementById("accessoryGridFull");
if (accessoryGridFull) {
  accessoryGridFull.innerHTML = loadingHTML("Loading accessories…");
  fetchAccessories().then((accessories) => {
    accessoryGridFull.innerHTML = accessories.length ? accessories.map(accessoryCard).join("") : `<p class="data-status">No accessories added yet.</p>`;
  }).catch(() => { accessoryGridFull.innerHTML = errorHTML(); });
}
