...
// Preference saved in localStorage so it persists across pages and visits.
(function initTheme() {
  const THEME_KEY = "bpTheme";
...
(function initHeroSlider() {
  ...
  if (!heroSlides || !heroDotsWrap) return;

  const slideCount = heroSlides.children.length;
  let heroIndex = 0;
  ...
  function goToHeroSlide(i) {
    ...
    heroDotsWrap.querySelectorAll(".hero-dot").forEach((d, idx) => d.classList.toggle("active", idx === i));
  }

  setInterval(() => { goToHeroSlide((heroIndex + 1) % slideCount); }, 5000);
})();
...
// Shimmer placeholder cards shown in product/laptop/accessory grids while
// data loads — replaces the old plain "Loading…" text with skeleton boxes
// shaped like the real cards.
...
if (trendingGrid || secondHandGrid) {
  if (trendingGrid) trendingGrid.innerHTML = skeletonGridHTML(4);
  if (secondHandGrid) secondHandGrid.innerHTML = skeletonGridHTML(4);
...
if (trendingLaptopsGrid) {
  trendingLaptopsGrid.innerHTML = skeletonGridHTML(4);
...
if (accessoryGrid) {
  accessoryGrid.innerHTML = skeletonGridHTML(4);
...
if (airbudsBlock) {
  ...
  accessoryGridOther.innerHTML = skeletonGridHTML(6);
...
if (productGrid) {
  ...
  productGrid.innerHTML = skeletonGridHTML(8);
...
if (laptopGrid) {
  ...
  laptopGrid.innerHTML = skeletonGridHTML(8);
...
// ===== PWA: register service worker (installable app + offline shell) =====
