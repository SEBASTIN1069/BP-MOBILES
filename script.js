...
function productCard(p) {
  ...
  return `
    <a href="product.html?id=${p.id}" class="product-card" data-brand="${p.brand}" data-price="${p.price}" data-name="${p.name.toLowerCase()}" data-condition="${p.condition}">
      ...
      <div class="product-specs">${p.ram} RAM · ${p.storage} · ${p.battery}</div>
...
// ===== Categorize an accessory by its "category" text into a section =====
// Matches loosely on keywords so the shop owner can type the category
// naturally in admin.html (e.g. "Airbuds", "Earbuds", "Fast Charger",
// "Back Cover") and it still lands in the right section here. Anything
// that doesn't match a known keyword falls into "other".
function accessorySectionFor(category) {
  const c = (category || "").toLowerCase();
  if (c.includes("airbud") || c.includes("earbud") || c.includes("earphone") || c.includes("headphone") || c.includes("neckband") || c.includes("bud")) {
    return "airbuds";
  }
  if (c.includes("charger") || c.includes("charging") || c.includes("cable") || c.includes("adapter")) {
    return "charger";
  }
  if (c.includes("cover") || c.includes("case") || c.includes("tempered") || c.includes("glass") || c.includes("screen guard") || c.includes("screenguard")) {
    return "cover";
  }
  return "other";
}

// ===== Render: accessories on products page (grouped by category) =====
...
if (airbudsBlock) {
  ...
  fetchAccessories().then((accessories) => {
    if (!accessories.length) {
      accessoryGridOther.innerHTML = "";
      accessoryEmptyState.hidden = false;
      return;
    }

    const grouped = { airbuds: [], charger: [], cover: [], other: [] };
    accessories.forEach((a) => grouped[accessorySectionFor(a.category)].push(a));

    if (grouped.airbuds.length) {
      airbudsBlock.hidden = false;
      accessoryGridAirbuds.innerHTML = grouped.airbuds.map(accessoryCard).join("");
    }
    if (grouped.charger.length) {
      chargerBlock.hidden = false;
      accessoryGridCharger.innerHTML = grouped.charger.map(accessoryCard).join("");
    }
    if (grouped.cover.length) {
...
function renderAuthPage(user) {
  ...
  signupForm.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      const passwordHash = await hashPassword(password);
      await saveCustomer(mobile, { name, mobile, passwordHash });
      localStorage.setItem(SESSION_KEY, mobile);
...
if (accessoryDetail) {
  ...
  db.collection("accessories").doc(accId).get().then((doc) => {
    if (!doc.exists) {
      accessoryDetail.innerHTML = `
        <div class="detail-not-found">
          ...
          <a href="products.html#accessories" class="btn btn-primary">Back to accessories</a>
        </div>
      `;
      return;
    }
    ...
    accessoryDetail.innerHTML = `
      <a href="products.html#accessories" class="back-link">← Back to accessories</a>
      <div class="product-detail-grid">
        ${visualHTML}
        <div class="detail-info">
          <div class="detail-top">
