...
function loadAccounts() {
  db.collection("customers").onSnapshot((snap) => {
    ...
    accountsTableBody.innerHTML = `<tr><td colspan="2">Couldn't load accounts.</td></tr>`;
  });
}
...
function renderAccountsPage() {
  ...
  if (filtered.length === 0) {
    accountsTableBody.innerHTML = `<tr><td colspan="2">${searchQuery ? "No matching accounts." : "No accounts created yet."}</td></tr>`;
